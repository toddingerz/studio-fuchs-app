...
import { 
  ChevronLeft, Copy, Check, Loader2, FileJson, Store, HeartPulse, Wrench, User, Sparkles, 
  HelpCircle, Lightbulb, Globe, Mic, Send, Briefcase, Inbox, ArrowRight, Square, UploadCloud, 
  Lock, Unlock, FileText, Pause, Play, Trash2, AlertCircle, Users, LayoutGrid
} from 'lucide-react';

// === CONFIGURATION ===

// Firebase Konfiguration wird zur Laufzeit geladen
let app, auth, db;
try {
  // Wenn wir auf Vercel sind, nutzen wir die Environment Variable
  // Wenn wir hier im Chat sind, nutzen wir die globale Variable __firebase_config
  const configSource = typeof process !== 'undefined' && process.env.REACT_APP_FIREBASE_CONFIG 
...
const CATEGORIES = [
  { id: "local", label: "Local Service Business", Icon: Store, examples: ["Friseur", "Tattoo", "Physio", "Gastro"] },
  ...
  { id: "brand", label: "Personal Brand / Experte", Icon: User, examples: ["Coach", "Berater", "Speaker"] }
];
...
export default function App() {
  ...
  const callAI = async (payload) => {
    try {
      // WICHTIG: Hier rufen wir DEIN Backend auf Vercel an
      // Wenn das hier in der Vorschau läuft, gibt es /api/gemini nicht -> Fehler
      ...
      if (!response.ok) {
        if (response.status === 404) throw new Error("Backend API nicht gefunden (Läuft die App auf Vercel?)");
        if (data.error === 'MISSING_API_KEY') throw new Error("Vercel Konfiguration fehlt (GEMINI_API_KEY).");
        throw new Error(data.message || data.error || "Fehler bei der KI-Anfrage");
      }
      
      return data;
    } catch (e) {
      // Fallback für Vorschau-Modus (nur für Demo-Zwecke hier im Chat)
      if (window.location.hostname.includes('googleusercontent')) {
         throw new Error("⚠️ VORSCHAU-MODUS: Die Analyse funktioniert nur auf der echten Vercel-Webseite, da der API-Key dort sicher im Backend liegt.");
      }
      throw e;
    }
  };
  ...
  // ... (Recording Logic identisch beibehalten für Kürze, hier der Fokus auf Fixes) ...
  const startRecording = async () => {
    setAppError(null);
    try {
      if (!navigator.mediaDevices) throw new Error("Kein Mikrofon-Zugriff.");
      const stream = await navigator.mediaDevices.getUserMedia({ audio: true });
      // Setup Audio Visualizer & Recorder...
    ...
    if (clientSubmitted) return (
      <div className="min-h-screen bg-gradient-to-br from-[#edd5e5] via-[#dcd2e6] to-[#c4c0e6] flex flex-col items-center justify-center p-6 text-center text-[#2c233e]">
        <div className="bg-white/60 backdrop-blur-xl p-16 rounded-[4rem] max-w-xl shadow-2xl border border-white/60">
          ...
          <p className="text-xl opacity-70 mb-10">Deine Daten sind sicher übermittelt.</p>
            ...
            <div className="lg:col-span-7 space-y-8">
              ...
              <div className="bg-white/50 border border-white/60 rounded-[3rem] overflow-hidden shadow-2xl relative">
                ...
                <div className="relative">
                    ...
                    <textarea value={transcript} onChange={e => setTranscript(e.target.value)} placeholder="Tippe deine Nachricht oder nutze die Aufnahme..." className="w-full bg-transparent p-12 text-xl font-medium min-h-[400px] outline-none resize-none leading-relaxed" />
                </div>
                <div className="p-8 border-t border-white/40 flex justify-end bg-white/10"><button onClick={handleClientSubmit} disabled={isSending || !transcript || !clientName} className="bg-[#e32338] text-white px-12 py-5 rounded-full font-bold uppercase tracking-widest text-sm shadow-xl disabled:opacity-30 flex items-center gap-3">{isSending ? <Loader2 className="animate-spin" /> : <><Send className="w-4 h-4" /> Analyse senden</>}</button></div>
              </div>
            </div>
          </div>
        </main>
