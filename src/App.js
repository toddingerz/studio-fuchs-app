...
export default function App() {
  ...
  const processAudio = async (file, targetSetter) => {
    ...
    reader.onload = async () => {
      ...
      try {
        const data = await callAI({ contents: [{ parts: [{ text: "Transkribiere dieses Audio wortwörtlich auf Deutsch. Antworte nur mit dem Text." }, { inlineData: { mimeType: file.type || 'audio/mp3', data: base64 } }] }] });
  ...
  const generateDNA = async () => {
    ...
    try {
      ...
      setStep(4);
        ...
        setAppError(String(err.message)); 
        setStep(2); 
    }
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-[#edd5e5] via-[#dcd2e6] to-[#c4c0e6] text-[#2c233e] font-sans selection:bg-[#e32338]/20">
        ...
        <main className="max-w-6xl mx-auto px-8 py-16 animate-in fade-in duration-700">
          ...
          {appError && <div className="max-w-xl mx-auto mb-8 bg-red-50 text-red-600 px-6 py-4 rounded-2xl font-bold">{String(appError)}</div>}
            ...
            <div className="lg:col-span-7 space-y-8">
              <div className="bg-white/40 border border-white/60 rounded-[3rem] p-10 shadow-xl space-y-4">
                ...
                <div className="space-y-2"><label className="text-[10px] font-bold uppercase opacity-40 ml-2">Bereich</label><div className="grid grid-cols-2 gap-2">
                    {CATEGORIES.map(c=><button key={c.id} onClick={()=>setClientCategory(c.label)} className={`p-3 rounded-2xl text-left border transition-all ${clientCategory === c.label ? 'bg-[#e32338] text-white border-transparent' : 'bg-white/30 border-white/60 text-[#2c233e]/60'}`}><div className="flex items-center gap-2 font-bold text-xs"><c.Icon className="w-4 h-4"/>{c.label}</div></button>)}
                    <button onClick={() => setClientCategory("Nicht sicher / Sonstiges")} className={`p-3 rounded-2xl text-center border text-xs ${clientCategory === "Nicht sicher / Sonstiges" ? 'bg-[#2c233e] text-white border-transparent' : 'bg-white/30 border-white/60 text-[#2c233e]/40'}`}>Ich bin mir nicht sicher</button>
                </div></div>
          ...
          <div className="mb-20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {submissions.length === 0 ? <div className="col-span-full border-2 border-dashed border-[#2c233e]/10 rounded-[4rem] p-24 text-center opacity-30 font-bold">Warte auf Kundendaten...</div> : submissions.map(sub => (
                <div key={sub.id} className={`bg-white/60 backdrop-blur-md border p-10 rounded-[3.5rem] shadow-lg hover:shadow-2xl transition-all relative ${activeClientName === sub.name ? 'border-[#e32338] ring-2 ring-[#e32338]/20' : 'border-white/60'}`}>
        ...
        {step === 4 && outputJson && (
          <div className="animate-in slide-in-from-bottom-8 duration-700 space-y-12">
             ...
             <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
               <div className="bg-[#2c233e] border border-white/10 rounded-[4rem] p-10 shadow-2xl overflow-auto max-h-[800px]">
                 <div className="flex justify-between items-center mb-6 border-b border-white/5 pb-4"><span className="text-white/40 text-[10px] font-bold uppercase tracking-widest">Base44 Schema</span><FileJson className="w-5 h-5 text-white/40" /></div>
