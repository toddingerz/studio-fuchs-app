...
const CATEGORIES = [
  {
    ...
    Icon: Store,
    examples: ["Friseur", "Tattoo", "Physio", "Gastro"]
  },
  {
    id: "med",
    label: "Medizin / Gesundheit",
    Icon: HeartPulse,
    examples: ["Arzt", "Zahnarzt", "Pflege"]
  },
  {
    id: "craft",
    label: "Handwerk / Produktion",
    Icon: Wrench,
    examples: ["Manufaktur", "Bau", "E-Com"]
  },
  {
    id: "brand",
    label: "Personal Brand / Experte",
    Icon: User,
    examples: ["Coach", "Berater", "Speaker"]
  }
];
...
export default function App() {
  ...
  const processAudio = async (file, targetSetter) => {
    ...
    reader.onload = async () => {
      ...
      try {
        const payload = {
            contents: [{ parts: [{ text: "Transkribiere dieses Audio wortwörtlich auf Deutsch. Antworte nur mit dem Text." }, { inlineData: { mimeType: file.type || 'audio/mp3', data: base64 } }] }]
        };
        ...
        setAppError("KI-Transkription fehlgeschlagen: " + err.message);
      }
  ...
  const handleClientSubmit = async () => {
    ...
    if (!db) { setAppError("Datenbank nicht verbunden."); return; }
    setIsSending(true);
    try {
    ...
    } catch (err) { setAppError("Senden fehlgeschlagen."); }
  ...
  const generateHooks = async () => {
    ...
    try {
      ...
      setSocialHooks(JSON.parse(cleanJsonResponse(raw)));
       ...
       console.error("Hook Fehler:", err);
       setAppError("Hooks konnten nicht erstellt werden.");
    }
  ...
  const generateStrategy = async () => {
    ...
    try {
      ...
      const data = await callProxyAPI(payload);
      const jsonResponse = extractJson(data.candidates?.[0]?.content?.parts?.[0]?.text);
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-[#edd5e5] via-[#dcd2e6] to-[#c4c0e6] text-[#2c233e] font-sans selection:bg-[#e32338]/20">
        ...
        <main className="max-w-6xl mx-auto px-8 py-16 animate-in fade-in duration-700">
          ...
          {appError && (
              <div className="max-w-xl mx-auto mb-8 bg-red-50 border border-red-200 text-red-600 px-6 py-4 rounded-2xl flex items-center gap-4 animate-in slide-in-from-top-2">
            ...
            <div className="lg:col-span-7 space-y-8">
              <div className="bg-white/40 border border-white/60 rounded-[3rem] p-8 shadow-xl space-y-4">
                ...
                <div className="space-y-4">
                    <label className="text-[10px] font-bold uppercase opacity-40 flex items-center gap-2"><Users className="w-4 h-4"/> Wie groß ist dein Team?</label>
                    <div className="flex flex-wrap gap-2">{COMPANY_SIZES.map(s => <button key={s} onClick={() => setCompanySize(s)} className={`px-5 py-2 rounded-xl text-xs font-bold transition-all border ${companySize === s ? 'bg-[#e32338] text-white border-transparent' : 'bg-white/30 border-white/60 text-[#2c233e]/50 hover:bg-white/50'}`}>{s}</button>)}</div>
                </div>

                {/* KATEGORIE */}
                <div className="space-y-4 pt-2">
                    <label className="text-[10px] font-bold uppercase opacity-40 flex items-center gap-2"><LayoutGrid className="w-4 h-4"/> Bereich wählen</label>
                    <div className="grid grid-cols-2 gap-3">
                        {CATEGORIES.map(c => (
                            <button key={c.id} onClick={() => setClientCategory(c.label)} className={`p-4 rounded-2xl text-left border transition-all flex flex-col gap-2 ${clientCategory === c.label ? 'bg-[#e32338] text-white border-transparent' : 'bg-white/30 border-white/60 text-[#2c233e]/60 hover:bg-white/50'}`}>
                                <div className="flex items-center gap-2 font-bold text-xs"><c.Icon className="w-4 h-4" /> {c.label}</div>
                            </button>
                        ))}
                ...
                <div className="grid grid-cols-2 gap-4 border-t border-[#2c233e]/5 pt-8">
                  <input type="url" value={clientWebsite} onChange={e => setClientWebsite(e.target.value)} placeholder="Website URL" className="bg-white/50 border border-white/60 rounded-2xl px-6 py-4 outline-none text-xs" />
                  <input type="text" value={clientSocial} onChange={e => setClientSocial(e.target.value)} placeholder="Instagram / LinkedIn" className="bg-white/50 border border-white/60 rounded-2xl px-6 py-4 outline-none text-xs" />
                </div>
              </div>
              
              <div className="bg-white/50 backdrop-blur-xl border border-white/60 rounded-[3rem] overflow-hidden shadow-2xl relative">
                <div className="p-8 flex flex-col items-center gap-6 bg-white/20 border-b border-white/40">
                  {!isRecording ? (
                    ...
                    <div className="w-full flex flex-col items-center gap-6">
                        <div className="w-full max-w-xs h-12 flex items-center justify-center gap-1">
                            {[...Array(20)].map((_, i) => (
                                <div key={i} className="w-1.5 bg-[#e32338] rounded-full transition-all duration-75" style={{ height: `${Math.max(15, (audioLevel * (0.5 + Math.random() * 1.5)))}%`, opacity: 0.3 + (audioLevel / 100)}} />
                            ))}
                        </div>
                        <div className="flex gap-4">
                            <button onClick={togglePause} className={`flex items-center gap-3 px-8 py-5 text-white rounded-full font-bold uppercase text-[11px] tracking-widest shadow-lg transition-all ${isPaused ? 'bg-emerald-500' : 'bg-amber-500'}`}>{isPaused ? <Play className="w-5 h-5"/> : <Pause className="w-5 h-5"/>} {isPaused ? "Weiter" : "Pause"}</button>
                            <button onClick={stopRecording} className="flex items-center gap-3 px-8 py-5 bg-[#2c233e] text-white rounded-full font-bold uppercase text-[11px] tracking-widest shadow-lg hover:bg-black"><Square className="w-5 h-5 fill-white" /> Fertig</button>
                        </div>
                    </div>
                  )}
                  {!isRecording && <label className="text-[10px] font-bold opacity-30 uppercase cursor-pointer hover:opacity-100 flex items-center gap-2"><UploadCloud className="w-4 h-4"/> Datei statt Sprache hochladen<input type="file" accept="audio/*" className="hidden" onChange={e => e.target.files[0] && processAudio(e.target.files[0], setTranscript)} /></label>}
                </div>
                <div className="relative">
                    {isTranscribing && <div className="absolute inset-0 bg-white/60 backdrop-blur-sm z-10 flex flex-col items-center justify-center p-10"><Loader2 className="w-10 h-10 text-[#e32338] animate-spin mb-4" /><p className="text-xs font-bold uppercase tracking-widest">KI schreibt Nachricht...</p></div>}
                    <textarea value={transcript} onChange={e => setTranscript(e.target.value)} placeholder="Tippe deine Nachricht hier ein (oder nutze die Aufnahme)..." className="w-full bg-transparent p-12 text-xl font-medium min-h-[400px] outline-none resize-none leading-relaxed placeholder:text-[#2c233e]/10" />
                </div>
                <div className="p-8 border-t border-white/40 flex justify-end bg-white/10"><button onClick={handleClientSubmit} disabled={isSending || !transcript || !clientName} className="bg-[#e32338] text-white px-12 py-5 rounded-full font-bold uppercase tracking-widest text-sm shadow-xl disabled:opacity-30 flex items-center gap-3 transform hover:translate-x-1 transition-all">{isSending ? <Loader2 className="animate-spin" /> : <><Send className="w-4 h-4" /> An Agentur senden</>}</button></div>
              </div>
            </div>
          </div>
        </main>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#edd5e5] via-[#dcd2e6] to-[#c4c0e6] text-[#2c233e] font-sans selection:bg-[#e32338]/20">
      ...
      <main className="max-w-7xl mx-auto px-8 py-16 animate-in fade-in duration-700">
          ...
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 space-y-12">
             ...
             <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
               <div className="lg:col-span-8 space-y-8">
                 <div className="bg-white/50 border border-white/60 rounded-[3rem] p-8 space-y-6 shadow-xl">
        ...
        {step === 3 && (
          <div className="h-[70vh] flex flex-col items-center justify-center text-center animate-in fade-in duration-700">
            ...
            <p className="text-xl opacity-40 font-medium">Extrahiere Brand DNA und Web-Intelligence...</p>
          </div>
        )}
          ...
          <div className="animate-in slide-in-from-bottom-8 duration-700 space-y-12">
             ...
             <div className="bg-white/50 backdrop-blur-xl border border-white/60 rounded-[4rem] p-12 shadow-2xl">
               <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-8 mb-10">
                 <div>
                   <h3 className="text-3xl font-bold mb-2 flex items-center gap-3"><FileText className="w-8 h-8 text-[#e32338]" /> Strategie <span className="text-[#e32338]">Report.</span></h3>
                   <p className="text-lg opacity-60 font-medium max-w-xl">Bonus Analyse im "Freystil Sales"-Format für den Kunden.</p>
                 </div>
                 {!strategyReport && (
                   <button onClick={generateStrategy} disabled={isGeneratingStrategy} className="px-10 py-5 bg-white text-[#2c233e] border-2 border-[#e32338]/10 rounded-full font-bold uppercase text-[11px] tracking-widest shadow-lg flex items-center gap-3 hover:bg-[#e32338] hover:text-white transition-all">
               ...
               {strategyReport && (
                 <div className="bg-white rounded-[3rem] p-12 border border-white/60 shadow-inner relative group animate-in slide-in-from-top-4 duration-500">
                    <button onClick={() => copySimpleText(strategyReport, () => alert("Report kopiert!"))} className="absolute top-8 right-8 p-4 bg-white hover:bg-[#e32338] hover:text-white rounded-full transition-all shadow-md active:scale-95"><Copy className="w-5 h-5" /></button>
                    <div className="prose prose-lg text-[#2c233e] whitespace-pre-wrap font-medium leading-relaxed max-w-none">
                      {strategyReport}
                    </div>
                 </div>
               )}
             ...
             <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
               <div className="bg-[#2c233e] border border-white/10 rounded-[4rem] p-10 shadow-2xl overflow-auto max-h-[800px] custom-scrollbar">
                 <div className="flex items-center justify-between mb-8 border-b border-white/5 pb-6 text-white/30 uppercase text-[10px] font-bold tracking-widest"><span>Base44 Schema</span><FileJson className="w-4 h-4" /></div>
      ...
      <style dangerouslySetInnerHTML={{__html: `
        ...
        .custom-scrollbar::-webkit-scrollbar-thumb { background: rgba(227, 35, 56, 0.2); border-radius: 10px; }
        .custom-scrollbar::-webkit-scrollbar-thumb:hover { background: rgba(227, 35, 56, 0.4); }
      `}} />
    </div>
  );
}
