...
import { 
  ...
  UploadCloud, Lock, Unlock, MousePointerClick, FileText, Pause, Play, Trash2, 
...
let app, auth, db;
// FIX: Sanitize appId to ensure it's a single path segment (replaces slashes that cause odd/even segment errors)
...
const PROXY_URL = "/api/gemini"; // Vercel Backend Proxy
...
const CATEGORIES = [
  { id: "local", label: "Local Service Business", Icon: Store },
  { id: "med", label: "Medizin / Gesundheit", Icon: HeartPulse },
  { id: "craft", label: "Handwerk / Produktion", Icon: Wrench },
  { id: "brand", label: "Personal Brand", Icon: User }
];
...
const STRATEGY_SYSTEM_INSTRUCTION = `Du bist Thorsten Fuchs, Sales-Mail-Architekt. Erstelle eine ehrliche "Freystil Sales"-Analyse direkt an den Kunden. Antworte im JSON Format mit einem Feld "report".`;
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(submissionsRef, (snapshot) => {
      ...
      setAppError("Zugriff verweigert oder Datenbankfehler.");
    });
    return () => unsubscribe();
  ...
  const callAI = async (payload) => {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(String(errData.error || `Server Fehler: ${response.status}`));
      }
        ...
        const data = await callAI({
          contents: [{ 
            parts: [
              ...
              { inlineData: { mimeType: 'audio/mp3', data: reader.result.split(',')[1] } }
            ] 
          }]
        });
  ...
  const handleDNAAnalyse = async () => {
    ...
    try {
      ...
      setOutputJson(JSON.parse(rawJson.replace(/```json|```/g, "").trim()));
      setStep(4);
    } catch (err) { 
      setAppError(String(err.message || "Analyse Fehler.")); 
      setStep(2); 
    } 
  ...
  if (appMode === 'login') return (
    <div className="min-h-screen bg-[#2c233e] flex items-center justify-center p-6 text-white text-center">
      <form onSubmit={(e) => { e.preventDefault(); if(pinInput === ADMIN_PIN) setIsAdminLoggedIn(true); else setLoginError(true); }} className="bg-white/10 p-12 rounded-[3rem] w-full max-w-md backdrop-blur-xl border border-white/10">
        ...
        {isAdminLoggedIn ? setAppMode('agency') : loginError && <p className="text-[#e32338] font-bold mb-4">Falsch.</p>}
