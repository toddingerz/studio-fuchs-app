...
// 1. Für Vercel (Produktion): Lass das hier leer! Der Server nutzt die Environment Variable.
// 2. Für Hier (Vorschau): Trag deinen Key ein, um es HIER zu testen.
...
export default function App() {
  ...
  const callAI = async (payload) => {
    // Wenn PREVIEW_API_KEY gesetzt ist, nutzen wir ihn (für hier)
    ...
    // Ansonsten versuchen wir den Vercel Proxy (für Live)
    try {
        ...
        // Wenn Proxy nicht da ist (lokal/preview ohne Key)
        if (response.status === 404) throw new Error("Kein API Key gefunden. Bitte auf Vercel Environment prüfen.");
